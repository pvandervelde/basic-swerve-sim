...
from swerve_controller.control import (
    BodyMotionCommand,
    DriveModuleMotionCommand,
    MotionCommand,
)
from swerve_controller.control_model import (
    DriveModuleDesiredValues,
    DriveModuleMeasuredValues,
)
from swerve_controller.drive_module import DriveModule
from swerve_controller.geometry import Point, RealNumberValueSpace
...
def get_drive_module_info(
    robot_length: float = 1.2,
    robot_width: float = 1.1,
    ...
    left_front = DriveModule(
        ...
        steering_axis_xy_position=Point(
            0.5 * (robot_length - 2 * wheel_radius),
            0.5 * (robot_width - wheel_width),
            0.0,
        ),
...
def get_linear_motion_profile(
    start: float, end: float, end_time: float, number_space: RealNumberValueSpace
) -> TransientVariableProfile:
    return SingleVariableLinearProfile(start, end, end_time, number_space)


def get_motions_from_ros_bag(bag_directory: str) -> List[MotionPlan]:
    result: List[MotionPlan] = []

    print("Reading from '{}' ...".format(bag_directory))

    initial_body_state: BodyState = BodyState(
        body_x_in_meters=0.0,
        body_y_in_meters=0.0,
        body_orientation_in_radians=0.0,
        body_angular_z_velocity_in_radians_per_second=0.0,
        body_linear_x_velocity_in_meters_per_second=0.0,
        body_linear_y_velocity_in_meters_per_second=0.0,
    ...
    initial_module_state: List[DriveModuleDesiredValues] = [
        DriveModuleDesiredValues(
            name="left-front",
            steering_angle_in_radians=0.0,
            drive_velocity_in_meters_per_second=0.0,
        ),
        DriveModuleDesiredValues(
            name="left-rear",
            steering_angle_in_radians=0.0,
    ...
    command_stack: deque[MotionCommand] = deque()

    bagpath = Path(bag_directory)
    ...
    with AnyReader([bagpath], default_typestore=typestore) as reader:
        ...
        previous_time_stamp_in_seconds: float = (
            float(end_time_in_nano_seconds - start_time_in_nano_seconds) / 1.0e9
        )

        # The time for a command is the time until the next command. So ideally we
        # would read the data store backwards (i.e. from the end to the start).
        messages = list(reader.messages(connections=connections))
        for i in range(1, len(messages)):
            connection, timestamp, rawdata = messages[-i]

            command_start_time_in_seconds: float = (
                float(timestamp - start_time_in_nano_seconds) / 1.0e9
            )
            command_duration_in_seconds: float = (
                previous_time_stamp_in_seconds - command_start_time_in_seconds
            )
    ...
    while command_stack:
        commands.append(command_stack.pop())

    plan = MotionPlan(
        description="Rosbag motion",
        name="rosbag",
...
def get_motions(input_files: List[str]) -> List[MotionPlan]:
    ...
    for input_file in input_files:
        ...
        with open(relative.absolute()) as f:
            ...
            initial_body_state: BodyState = BodyState(
                body_x_in_meters=data_initial_body_state[
                    "position_in_meters_relative_to_world"
                ]["x"],
                body_y_in_meters=data_initial_body_state[
                    "position_in_meters_relative_to_world"
                ]["y"],
                body_orientation_in_radians=data_initial_body_state[
                    "orientation_in_radians_relative_to_world"
                ]["z"],
                body_angular_z_velocity_in_radians_per_second=data_initial_body_state[
                    "angular_velocity_in_radians_per_second"
                ]["z"],
                body_linear_x_velocity_in_meters_per_second=data_initial_body_state[
                    "linear_velocity_in_meters_per_second"
                ]["x"],
                body_linear_y_velocity_in_meters_per_second=data_initial_body_state[
                    "linear_velocity_in_meters_per_second"
                ]["y"],
                body_angular_z_acceleration_in_radians_per_second_quared=0.0,
                body_linear_x_acceleration_in_meters_per_second_quared=0.0,
            ...
            for module_initial_state in data_initial_module_state:
                state = DriveModuleDesiredValues(
                    ...
                    steering_angle_in_radians=module_initial_state[
                        "orientation_in_radians_relative_to_body"
                    ],
                    drive_velocity_in_meters_per_second=module_initial_state[
                ...
                if "modules" in data_command:
                    ...
                    for module_command_state in data_command_module:
                        state = DriveModuleDesiredValues(
                            ...
                            steering_angle_in_radians=module_command_state[
                                "orientation_in_radians_relative_to_body"
                            ],
                            drive_velocity_in_meters_per_second=module_command_state[
                                "velocity_in_meters_per_second"
                            ],
                        )
                    ...
                    if "body" in data_command:
                        ...
                        command = BodyMotionCommand(
                            time_span,
                            data_command_body["linear_velocity_in_meters_per_second"][
                                "x"
                            ],
                            data_command_body["linear_velocity_in_meters_per_second"][
                                "y"
                            ],
                            data_command_body["angular_velocity_in_radians_per_second"][
                                "z"
                            ],
                        )
...
def get_scurve_profile(
    start: float, end: float, end_time: float, number_space: RealNumberValueSpace
) -> TransientVariableProfile:
    return SingleVariableSCurveProfile(start, end, end_time, number_space)


def get_trapezoidal_profile(
    start: float, end: float, end_time: float, number_space: RealNumberValueSpace
) -> TransientVariableProfile:
    return SingleVariableTrapezoidalProfile(start, end, end_time, number_space)


def initialize_drive_modules(
    drive_modules: List[DriveModule], module_states: List[DriveModuleDesiredValues]
...
def initialize_state_file(file_path: str, number_of_modules: int):
    with open(file_path, mode="w") as file_:
        ...
        file_.write(
            "x-rot-body [wc] (rad),y-rot-body [wc] (rad),z-rot-body [wc] (rad),"
        )

        file_.write(
            "x-vel-body [bc] (m/s), y-vel-body [bc] (m/s), z-vel-body [bc] (m/s),"
        )
        file_.write(
            "x-rotvel-body [bc] (rad/s), y-rotvel-body [bc] (rad/s), z-rotvel-body [bc] (rad/s),"
        )

        file_.write(
            "x-acc-body [bc] (m/s^2), y-acc-body [bc] (m/s^2), z-acc-body [bc] (m/s^2),"
        )
        file_.write(
            "x-rotacc-body [bc] (rad/s^2), y-rotacc-body [bc] (rad/s^2), z-rotacc-body [bc] (rad/s^2),"
        )

        file_.write(
            "x-jerk-body [bc] (m/s^3), y-jerk-body [bc] (m/s^3), z-jerk-body [bc] (m/s^3),"
        )
        file_.write(
            "x-rotjerk-body [bc] (rad/s^3), y-rotjerk-body [bc] (rad/s^3), z-rotjerk-body [bc] (rad/s^3),"
        )

        file_.write("number of modules (-),")
        for i in range(number_of_modules):
            file_.write(
                f"x-module-{i} [bc] (m), y-module-{i} [bc] (m), z-module-{i} [bc] (m),"
            )
            file_.write(
                f"x-rot-module-{i} [bc] (rad), y-rot-module-{i} [bc] (rad), z-rot-module-{i} [bc] (rad),"
            )

            file_.write(
                f"x-vel-module-{i} [mc] (m/s), y-vel-module-{i} [mc] (m/s), z-vel-module-{i} [mc] (m/s),"
            )
            file_.write(
                f"x-rotvel-module-{i} [bc] (rad/s), y-rotvel-module-{i} [bc] (rad/s), z-rotvel-module-{i} [bc] (rad/s),"
            )

            file_.write(
                f"x-acc-module-{i} [mc] (m/s^2), y-acc-module-{i} [mc] (m/s^2), z-acc-module-{i} [mc] (m/s^2),"
            )
            file_.write(
                f"x-rotacc-module-{i} [bc] (rad/s^2), y-rotacc-module-{i} [bc] (rad/s^2), z-rotacc-module-{i} [bc] (rad/s^2),"
            )

            file_.write(
                f"x-jerk-module-{i} [mc] (m/s^3), y-jerk-module-{i} [mc] (m/s^3), z-jerk-module-{i} [mc] (m/s^3),"
            )
            file_.write(
                f"x-rotjerk-module-{i} [bc] (rad/s^3), y-rotjerk-module-{i} [bc] (rad/s^3), z-rotjerk-module-{i} [bc] (rad/s^3),"
            )
...
def record_state_at_time(
    ...
    with open(file_path, mode="a") as file_:
        ...
        file_.write(
            "{},{},{},".format(body_linear_vel.x, body_linear_vel.y, body_linear_vel.z)
        )

        body_angular_vel = body_state.motion_in_body_coordinates.angular_velocity
        file_.write(
            "{},{},{},".format(
                body_angular_vel.x, body_angular_vel.y, body_angular_vel.z
            )
        )
...
def simulation_run_trajectories_from_motion_files(arg_dict: Mapping[str, any]):
...
def simulation_run_trajectories_from_rosbag_file(arg_dict: Mapping[str, any]):
    input_dir: str = arg_dict["directory"]
    output_directory: str = arg_dict["output"]
    do_not_draw_graphs: bool = arg_dict["no_graphs"]
    controller: str = arg_dict["control_level"]
    motion_profile: str = arg_dict["motion_profile"]
    print("Running trajectory simulation")
    print("Simulating motion for the following ros bag:")
    print("    {}".format(input_dir))

    print("Outputting to {}".format(output_directory))

    drive_modules = get_drive_module_info()
    motions = get_motions_from_ros_bag(input_dir)
    for motion_set in motions:
        simulation_run_trajectory(
            output_directory,
            drive_modules,
...
def simulation_run_trajectory(
    ...
    if controller_name == "body":
        controller = ModuleFollowsBodySteeringController(
            drive_modules, motion_profile_func
        )

    if controller_name == "limited":
        controller = LimitedModuleFollowsBodySteeringController(
            drive_modules, motion_profile_func, 100
        )

    motion_directory = path.join(
        output_directory, motion_set.name, controller_name, motion_profile
    )

    state_file_path = path.join(motion_directory, "sim_results.csv")
    if not path.isdir(motion_directory):
        print(
            "Output directory {} does not exist. Creating directory ...".format(
                motion_directory
            )
        )
    ...
    initial_module_states: List[DriveModuleMeasuredValues] = initialize_drive_modules(
        drive_modules, motion_set.initial_drive_module_states
    )

    controller.on_state_update(initial_module_states)

    simulation_rate_in_hz = 10
    ...
    for motion in motion_set.motions:
        ...
        # If the time_for_motion * simulation_rate_in_hz is less than 3 then we wouldn't
        # have enough steps to simulate the motion. So we'll set the number of steps to
        # at least 3
        step_count = 1
        time_step_in_seconds = time_for_motion / 1.0
        number_of_steps_in_motion = time_for_motion * simulation_rate_in_hz
        if number_of_steps_in_motion > 1.0:
            step_count = int(number_of_steps_in_motion)
        ...
        for _ in range(1, step_count + 1):
            ...
            current_drive_module_states = (
                controller.drive_module_states_at_current_time()
            )
            icr_coordinate_map = instantaneous_center_of_rotation_at_current_time(
                current_drive_module_states
            )
            icr_map.append((current_sim_time_in_seconds, icr_coordinate_map))
            ...
            desired_drive_module_states = controller.drive_module_state_at_future_time(
                current_sim_time_in_seconds
            )
            ...
            for module_index in range(len(drive_modules)):
                orientation_velocity = (
                    ...
                    - current_drive_module_states[
                        module_index
                    ].orientation_in_body_coordinates.z
                ) / time_step_in_seconds
                orientation_acceleration = (
                    orientation_velocity
                    - current_drive_module_states[
                        module_index
                    ].orientation_velocity_in_body_coordinates.z
                ) / time_step_in_seconds
                orientation_jerk = (
                    orientation_acceleration
                    - current_drive_module_states[
                        module_index
                    ].orientation_acceleration_in_body_coordinates.z
                ) / time_step_in_seconds

                drive_acceleration = (
                    desired_drive_module_states[
                        module_index
                    ].drive_velocity_in_meters_per_second
                    - current_drive_module_states[
                        module_index
                    ].drive_velocity_in_module_coordinates.x
                ) / time_step_in_seconds
                drive_jerk = (
                    drive_acceleration
                    - current_drive_module_states[
                        module_index
                    ].drive_acceleration_in_module_coordinates.x
                ) / time_step_in_seconds
    ...
    print("Simulation required {} steps".format(total_step_count))
...
def main(args=None):
    ...
    if arg_dict["file"] is not None:
        simulation_run_trajectories_from_motion_files(arg_dict)
    else:
        simulation_run_trajectories_from_rosbag_file(arg_dict)
